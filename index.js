...
app.get('/api/quiz/:name', async (req, res) => {
  ...
  try {
    ...
    if (!data || data.length === 0) return res.status(400).json({ error: 'Spreadsheet is empty' });

    // Dynamically find the header row (the first row containing 'question')
    const headerRowIndex = data.findIndex(row => 
      row.some(cell => cell && String(cell).toLowerCase().trim() === 'question')
    );

    if (headerRowIndex === -1) return res.status(400).json({ error: 'Sheet format invalid: "question" column not found' });

    const headers = data[headerRowIndex].map(h => String(h).trim().toLowerCase());
    const qIdx = headers.indexOf('question');
    const oIdx = headers.indexOf('options');
    const aIdx = headers.indexOf('answer');
    const iIdx = headers.indexOf('image');

    const questions = data.slice(headerRowIndex + 1).map(row => {
      if (!row[qIdx]) return null;
      const optionsStr = row[oIdx] || "";
      return {
        question: row[qIdx],
        options: optionsStr ? optionsStr.split(',').map(opt => opt.trim()).filter(Boolean) : [],
        answer: row[aIdx] || "",
        image: row[iIdx] || ''
      };
    }).filter(Boolean);

    res.json({ quizName: data[0][0] || quizName, questions });
  } catch (err) {
...
app.post('/api/submit', async (req, res) => {
  ...
  try {
    ...
    const sheetExists = spreadsheet.data.sheets.some(s => s.properties.title === resultsTab);

    if (!sheetExists) {
      await sheets.spreadsheets.batchUpdate({
        spreadsheetId: sid,
        requestBody: {
          requests: [{ addSheet: { properties: { title: resultsTab } } }]
        }
      });
      // Add Headers
      await sheets.spreadsheets.values.update({
        spreadsheetId: sid,
        range: `${resultsTab}!A1:F1`,
        valueInputOption: 'USER_ENTERED',
        requestBody: {
          values: [['Timestamp', 'Student Name', 'Register Number', 'Department', 'Year', 'Score']]
        }
      });
    }
